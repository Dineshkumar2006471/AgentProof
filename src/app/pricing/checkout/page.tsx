...
export default async function CheckoutPage({ searchParams }: { searchParams: Promise<{ plan?: string }> }) {
  ...
  return <AppShell title="Checkout" section="BILLING"><div className="workspace-page checkout-page">
    ...
    <KpiGrid metrics={[{ label: "Selected plan", value: plan.name, detail: plan.recurring ? "Monthly subscription" : "One-time purchase" }, { label: "Price", value: plan.price, detail: plan.recurring ? "Billed monthly" : "Charged once", tone: "pass" }, { label: "Capacity", value: plan.quota, detail: "Included verification usage" }, { label: "Reports", value: plan.reports, detail: "Available after verified payment" }]} />
