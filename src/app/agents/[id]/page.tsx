...
export default async function AgentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <AppShell title={agent.name} section="AGENT DOSSIER"><div className="workspace-page">
    <PageHeader eyebrow="AGENT DOSSIER" title={agent.name} description="Operational health, contract coverage, and the latest verification evidence for this deployment." meta={<div className="flex flex-wrap gap-x-5 gap-y-2 mono"><span>VERSION / {agent.currentVersion}</span><span>ENDPOINT / {agent.endpointUrl}</span>{latest && <StatusPill status={latest.status} />}</div>} actions={<><ActionButton href={`/agents/${agent.id}/run`}>Run verification</ActionButton>{reportHref && <ActionButton variant="quiet" href={reportHref} icon={null}>Public report</ActionButton>}</>} />
    <KpiGrid metrics={[{ label: "Reliability score", value: score === undefined ? "--" : `${score}/100`, detail: latest ? "Latest verification" : "No completed run", tone: latest?.status === "BLOCKED" || latest?.status === "FAILED" ? "fail" : "pass" }, { label: "Tests executed", value: latest ? String(latest.totalTests) : "--", detail: latest ? `${latest.passed} passed` : "Awaiting first run" }, { label: "Critical failures", value: latest ? String(latest.critical) : "--", detail: latest?.critical ? "Requires attention" : "No critical findings", tone: latest?.critical ? "fail" : "pass" }, { label: "Valid until", value: latest?.validUntil ?? "--", detail: latest ? "Report validity" : "Not verified" }]} />
